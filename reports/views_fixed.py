    ...
    def get(self, request, report_id):
        ...
        if role == "patient":
            try:
                ...
                if report.status != "downloadable":
                    return Response({"error": "Report not downloadable"}, status=403)
            except Patient.DoesNotExist:
                return Response({"error": "Patient not found"}, status=403)
        ...
        try:
            ...
            return Response({"error": "File read error"}, status=500)
